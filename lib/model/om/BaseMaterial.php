...
abstract class BaseMaterial extends BaseObject 
{
    ...
    /**
     * @var        PropelObjectCollection|Carrera[] Collection to store aggregation of Carrera objects.
     */
     ...
     */
    protected $carrerasScheduledForDeletion = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->carrerasScheduledForDeletion !== null) {
                if (!$this->carrerasScheduledForDeletion->isEmpty()) {
                    CarreraQuery::create()
                        ->filterByPrimaryKeys($this->carrerasScheduledForDeletion->getPrimaryKeys(false))
                        ->delete($con);
                    $this->carrerasScheduledForDeletion = null;
                }
            }
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collCarreras) {
                $result['Carreras'] = $this->collCarreras->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getCarreras() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addCarrera($relObj->copy($deepCopy));
                }
            }
    ...
    {
        if ('Carrera' == $relationName) {
            $this->initCarreras();
        }
    }

    /**
     * Clears out the collCarreras collection
     *
    ...
    {
        $this->collCarreras = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Initializes the collCarreras collection.
     *
     * By default this just sets the collCarreras collection to an empty array (like clearcollCarreras());
     ...
     * @return void
     */
    public function initCarreras($overrideExisting = true)
    {
        if (null !== $this->collCarreras && !$overrideExisting) {
            return;
        }
        $this->collCarreras = new PropelObjectCollection();
        $this->collCarreras->setModel('Carrera');
    }
     ...
     * @param      PropelPDO $con optional connection object
     * @return PropelObjectCollection|Carrera[] List of Carrera objects
     * @throws PropelException
     */
    public function getCarreras($criteria = null, PropelPDO $con = null)
    {
        if (null === $this->collCarreras || null !== $criteria) {
            if ($this->isNew() && null === $this->collCarreras) {
                // return empty collection
                $this->initCarreras();
            } else {
                $collCarreras = CarreraQuery::create(null, $criteria)
                    ->filterByMaterial($this)
                    ->find($con);
                if (null !== $criteria) {
                    return $collCarreras;
                }
                $this->collCarreras = $collCarreras;
            }
        }
    ...
    /**
     * Sets a collection of Carrera objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      PropelCollection $carreras A Propel collection.
     * @param      PropelPDO $con Optional connection object
     */
    public function setCarreras(PropelCollection $carreras, PropelPDO $con = null)
    {
        $this->carrerasScheduledForDeletion = $this->getCarreras(new Criteria(), $con)->diff($carreras);

        foreach ($this->carrerasScheduledForDeletion as $carreraRemoved) {
            $carreraRemoved->setMaterial(null);
        }
    ...
    public function countCarreras(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        if (null === $this->collCarreras || null !== $criteria) {
            if ($this->isNew() && null === $this->collCarreras) {
                ...
                $query = CarreraQuery::create(null, $criteria);
                if ($distinct) {
                    $query->distinct();
                }
            ...
            return count($this->collCarreras);
        }
    }
    ...
    public function addCarrera(Carrera $l)
    {
        ...
        if (!$this->collCarreras->contains($l)) { // only add it if the **same** object is not already associated
            $this->doAddCarrera($l);
        }
    ...
    /**
     * @param	Carrera $carrera The carrera object to add.
     */
    protected function doAddCarrera($carrera)
    {
        $this->collCarreras[]= $carrera;
        $carrera->setMaterial($this);
    }

    /**
     * @param	Carrera $carrera The carrera object to remove.
     */
    public function removeCarrera($carrera)
    {
        if ($this->getCarreras()->contains($carrera)) {
            $this->collCarreras->remove($this->collCarreras->search($carrera));
            if (null === $this->carrerasScheduledForDeletion) {
                $this->carrerasScheduledForDeletion = clone $this->collCarreras;
                $this->carrerasScheduledForDeletion->clear();
            }
            $this->carrerasScheduledForDeletion[]= $carrera;
            $carrera->setMaterial(null);
        }
    }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep) {
            if ($this->collCarreras) {
                foreach ($this->collCarreras as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        } // if ($deep)

        if ($this->collCarreras instanceof PropelCollection) {
            $this->collCarreras->clearIterator();
        }
        $this->collCarreras = null;
        $this->aSubcontenido = null;
    }
